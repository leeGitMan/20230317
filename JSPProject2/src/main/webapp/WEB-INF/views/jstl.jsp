...
<%--
	...
	prefix : 접두사
	uri(Uniform Resource Identifier) : 자원을 구분하는 식별자 (주소형태)
...
<body>
    ...
    <h3>JSTL 사용하기 위한 선언 방법</h3>
    
    <pre>
    
    	JSP 파일 최상단에 추가하고자 하는, JSTL taglib 추가
    ...
    <table border="1"> 
    	<c:forEach var="n" begin="1" end="10" varStatus="vs">
    		<c:choose>
    			<%-- choose 내부에는 무조건 JSP 주석만 작성 가능 --%>
    			<%-- 첫 번째 반복일 경우 --%>
    			<c:when test="${ vs.first }">
    				<tr>
    					<th class="first">${ n }</th>
    					<td class="first">${ n번 게시글 입니다. }</td>
    				</tr>
    			</c:when>
