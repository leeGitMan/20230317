...
public class SendEmailServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String subject = "[Community 프로젝트] 회원 가입 이메일 인증번호"; // 이메일 타이틀
		
		String fromEmail = "cmhinst@gmail.com"; // 보내는 사람으로 표시될 이메일
		
		String fromUserName = "관리자"; // 보내는 사람이름		
		
		String toEmail = inputEmail; // 받는사람, 콤마(,)로 여러개 나열 가능
		
		// 구글 이메일을 이용한 메일 보내기(SMPT : Simple Mail Transfer Protocol)
		
		// 1. 구글 계정 생성(기존 이메일 사용 가능)
		// 2. 계정 -> 보안 설정 진행
		// 		1) 2단계 인증 추가
		// 		2) 앱 비밀번호 생성(메일, 서버 컴퓨터 OS) -> 저장해두기  [mffxisgaykveneqv]
		
		
		final String smtpEmail = "gosrod05@gmail.com"; // 이메일
		final String password = "mffxisgaykveneqv"; // 발급받은 비밀번호
		...
		props.put("mail.smtp.auth", "true"); // 기본 값 false
		...
		try {
			...
			// 파라미터로 전달받은 Properties에 저장되어 있는 속성 값을 사용해서 세션을 생성
			
			// 메일 송/수신 옵션 설정
			Message message = new MimeMessage(session);
			message.setFrom(new InternetAddress(fromEmail,fromUserName)); // 송신자(보내는 사람) 지정
			message.addRecipient(RecipientType.TO, new InternetAddress(toEmail)); // 수신자 (받는 사람) 지정
			...
			// ajax error 속성 활용을 위한 500에러를 응답전달
